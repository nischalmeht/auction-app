...
const superAdminSlice = createSlice({
  ...
  export const getAllUsers  = () => async (dispatch) => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:3000/api/super-admin/user/getalluser",
        { withCredentials: true }
      );
      dispatch(
        superAdminSlice.actions.successForMonthlyRevenue(
  ...
  export const getAllPaymentProofs   = () => async (dispatch) => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:3000/api/super-admin/getallpaymentproof",
        { withCredentials: true }
      );
      dispatch(
  ...
  export const deletePaymentProof    = () => async (dispatch) => {
    dispatch(superAdminSlice.actions.requestForDeletePaymentProof());
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/super-admin/deletePaymentProof/:${id}`,
        { withCredentials: true }
      );
      dispatch(
        superAdminSlice.actions.successForDeletePaymentProof(
            response.data.paymentProofs
        )
      );y
    } catch (error) {
  ...
  export const getSinglePaymentProofDetail = (id) => async (dispatch) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/api/super-admin/getPaymentProofById/${id}`,
        { withCredentials: true }
      );
      dispatch(
  ...
  export const updatePaymentProof = (id, status, amount) => async (dispatch) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/api/super-admin/updatePaymentProof/${id}`,
        { status, amount },
  ...
  export const deleteAuctionItem = (id) => async (dispatch) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/super-admin/${id}`,
        { withCredentials: true }
      );
